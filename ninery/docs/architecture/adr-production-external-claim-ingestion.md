...
Legacy pre-Ticket #076 qualification compatibility is bounded by an immutable migration-time exemption ledger. The PostgreSQL migration explicitly opens a transaction and takes an `ACCESS EXCLUSIVE` lock on the qualification table before capturing the snapshot. It creates and seals the ledger and installs lineage enforcement before committing and releasing that lock. A null-lineage qualification is accepted only when its ID is in that protected snapshot. Every qualification serialized after the snapshot boundary must therefore provide both source and governance revision, and both must match its normalized claim's document source.
