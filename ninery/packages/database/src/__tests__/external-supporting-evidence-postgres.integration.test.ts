...
async function graph(options: Options = {}) {
  ...
  const governance = await db.externalEvidenceSourceGovernanceRevision.create({ data: { sourceId: source.id, revisionNumber: 1, governanceVersion: "1.0", sourceType: source.sourceType, authorityScope: { claimAuthority: "observational" }, dependencyKnowledge: { state: "claim_level_assessment_required" }, state: "active", reviewerType: "system", reviewerReference: "ticket-075-postgres-fixture", rationale: "Fixture governance establishes valid post-Ticket-076 qualification lineage.", effectiveAt: new Date(), idempotencyKey: randomUUID() } });
