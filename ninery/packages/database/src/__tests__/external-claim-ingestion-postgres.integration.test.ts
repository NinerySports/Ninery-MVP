...
integration("Ticket #076 production ingestion PostgreSQL boundary", async () => {
  ...
  const historicalVariantQualification = { ...first.succeeded[0]!.qualification, proposedEvidenceInput: persistedQualificationRow.proposedEvidenceInput ?? undefined };
  const reconstructedVariantFingerprint = qualificationFingerprint({ qualification: historicalVariantQualification, normalizedClaimId: first.succeeded[0]!.normalizedClaimId, rawClaimId: first.succeeded[0]!.rawClaimId, identityAssertionId: first.succeeded[0]!.identityAssertionId, dependencyAssessmentId: first.succeeded[0]!.dependencyAssessmentId, constructRelationshipId: first.succeeded[0]!.constructRelationshipId, sourceGovernanceRevisionId: first.succeeded[0]!.reviewReady.historicalSourceGovernanceRevisionId, authority: first.succeeded[0]!.qualification.authority! });
  ...
  const historicalEquipmentQualification = { ...equipmentLevelRecord.qualification, proposedEvidenceInput: persistedEquipmentQualificationRow.proposedEvidenceInput ?? undefined };
  const reconstructedEquipmentFingerprint = qualificationFingerprint({ qualification: historicalEquipmentQualification, normalizedClaimId: equipmentLevelRecord.normalizedClaimId, rawClaimId: equipmentLevelRecord.rawClaimId, identityAssertionId: equipmentLevelRecord.identityAssertionId, dependencyAssessmentId: equipmentLevelRecord.dependencyAssessmentId, constructRelationshipId: equipmentLevelRecord.constructRelationshipId, sourceGovernanceRevisionId: equipmentLevelRecord.reviewReady.historicalSourceGovernanceRevisionId, authority: equipmentLevelRecord.qualification.authority! });
  assert.equal(reconstructedEquipmentFingerprint, persistedEquipmentQualificationRow.semanticFingerprint);
  const alteredFingerprint = qualificationFingerprint({ qualification: { ...historicalVariantQualification, state: "not_eligible" }, normalizedClaimId: first.succeeded[0]!.normalizedClaimId, rawClaimId: first.succeeded[0]!.rawClaimId, identityAssertionId: first.succeeded[0]!.identityAssertionId, dependencyAssessmentId: first.succeeded[0]!.dependencyAssessmentId, constructRelationshipId: first.succeeded[0]!.constructRelationshipId, sourceGovernanceRevisionId: first.succeeded[0]!.reviewReady.historicalSourceGovernanceRevisionId, authority: first.succeeded[0]!.qualification.authority! });
